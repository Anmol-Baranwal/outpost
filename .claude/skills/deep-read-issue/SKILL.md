...
- ## A. Issue deep reads — one per issue, LEAD each with the `STATUS AS OF <today>:` line, the `COMMUNITY: CK|AG-UI (MIS-FILED?)` line, and the `THREADS/PERSISTENCE: YES (💰 paid boundary?) | NO` line, then maintainer-status quote, repro, refs.
  - **`THREADS/PERSISTENCE`** — `YES` when the issue's **subject** is a chat thread (lifecycle, locking, reload/restore, message ordering, history replay, thread-scoped state, thread caps, resume across sessions) or **persistence in any form** (retention, storage, durability, snapshot restore, resume/replay, session/state persistence, paying for persistence). `NO` for an incidental `threadId` in a stack trace, thread-safety/concurrency/worker threads, or "thread" meaning a Discord forum thread or GitHub discussion. Judge what the issue is *about* — a keyword match returns ~70% of CopilotKit issues and is useless here. Add `💰` when it crosses a **paid/commercial surface** — the thread cap, the retention window or the storage limit; **managed CopilotKit Intelligence**; a **managed Channel** (Slack/Teams); **self-hosted Enterprise Intelligence under a licence key**; managed or durable persistence; or paying for persistence at all. The test is whether the reporter is on a paid surface, **managed or licensed self-host** — not whether the feature also exists free, since a feature metered on every tier can still produce a paid-surface defect. This line feeds the report's standing 🧵 Threads & Persistence watch, so a missed `YES` drops an item the business is tracking.
...
- [ ] Every issue has a `THREADS/PERSISTENCE:` line — `YES` (with 💰 when it crosses a paid surface: thread cap, retention, storage, managed Intelligence, a managed Channel, licensed self-host Intelligence, or managed/durable persistence) or `NO`, decided on what the issue is ABOUT, not on the word appearing in it. A missing or wrongly-`NO` line drops the item from the report's 🧵 watch.
