...
- **Say whether the GitHub profile ITSELF corroborates the published name and employer — a reader will compare the two links side by side.** The card gives a reader two clickable identities: the GitHub handle and the LinkedIn. **If the GitHub profile is bare, they won't visibly match, and the card must say so instead of presenting the chain as settled.** State which source each fact came from, in order, and name the link in the chain that is missing.
  (Precedent 2026-08-14: `arslan-autoscout24` was published as "Arslan Mehboob" linked to a LinkedIn profile. The evidence was genuinely good — corporate commit email `arslan.mehboob@autoscout24.com`, and AutoScout24's own engineering blog bylines that exact name and links that exact profile. But his **GitHub profile reads `Arslan` with no surname, no company, no bio and no self-linked accounts**, so the surname appears nowhere on GitHub — it comes from an email local-part and an employer blog. A reader comparing the GitHub link to the LinkedIn link reasonably reported them as not matching. The facts were right; presenting them as a settled identity was not.)

- **Never let an email local-part stand in for a published full name.** `arslan.mehboob@…` strongly suggests a surname; it does not publish one. Cite an actual byline, self-owned page, or self-linked profile for the name — and if the only surname evidence is an email address, say that explicitly on the card.

- **An employer-published LinkedIn is good evidence of the *person*, weaker evidence of their *current* role.** A blog byline can predate a job change, and LinkedIn itself cannot be fetched (HTTP 999), so the profile's current employer and title are unverified whenever the link came from a third party rather than the person. Mark those **verify before outreach** even when the name is solid.
