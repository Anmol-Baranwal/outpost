...
- **Row:** link first (mandatory), one sentence on the subject, a 💰 marker when it crosses the paid boundary — thread cap, retention, storage, managed Intelligence, a **managed Channel**, **licensed self-host Intelligence**, managed/durable persistence, or paying for persistence (a feature metered on every tier can still yield a paid-surface defect; the test is whether the reporter is on a paid surface — managed or licensed self-host), and a pointer to the full card when the item appears elsewhere. Resolved items stay in, marked ✅ with what closed them.
