...
export function loadNamedIds(env: NodeJS.ProcessEnv = process.env): Map<string, string> {
    ...
    for (const [key, value] of Object.entries(env)) {
        ...
        if (/^\d+$/.test(alias)) {
            throw new Error(
                `Discord id alias "${alias}" is all digits — resolveId treats all-digit tool args as raw ` +
                    `snowflake ids, so a numeric alias name could never be looked up. Give the ` +
                    `DISCORD_GUILD_/DISCORD_CHANNEL_ var a non-numeric name.`,
            );
        }
