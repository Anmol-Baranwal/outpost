...
describe("loadNamedIds", () => {
    ...
    it("throws on a purely-numeric alias name (it would collide with raw-id passthrough)", () => {
        // resolveId treats an all-digit arg as a raw snowflake id, so a numeric alias
        // name could never be resolved — reject it at config load rather than fail silently.
        expect(() =>
            loadNamedIds({ DISCORD_GUILD_123: "456" } as unknown as NodeJS.ProcessEnv),
        ).toThrow(/all digits/);
    });
});
