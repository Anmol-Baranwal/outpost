...
# Neither prisma invocation is bounded by default. Against a black-holing endpoint
# (a dropped packet rather than a refusal) the CLI can hang far past any TCP
# timeout, and a hang produces NO log line at all — indistinguishable from a slow
# migration and from a healthy-but-slow boot, which defeats the point of putting
# the reason in the deploy log. `timeout` is a busybox applet, present in alpine.
STEP_TIMEOUT="${OUTPOST_STARTUP_STEP_TIMEOUT:-180}"

if command -v timeout >/dev/null 2>&1; then
    run_step() { timeout "$STEP_TIMEOUT" "$@"; }
else
    ...
    echo "[worker] NOTE: timeout(1) is unavailable; startup steps will run unbounded." >&2
    run_step() { "$@"; }
fi

# Everything diagnostic goes to stderr. web and worker migrate into the same
# database and their Railway logs interleave, so severity has to be
# machine-distinguishable and not merely prefixed — log shippers and alert rules
# key off the stream, not the text.
fatal() {
    echo "" >&2
    while [ "$#" -gt 0 ]; do
        echo "[worker] $1" >&2
...
drift_override_set() {
    value=$(printf '%s' "${OUTPOST_ALLOW_SCHEMA_DRIFT:-0}" | tr '[:upper:]' '[:lower:]' | tr -d '[:space:]"')
    case "$value" in
...
set +e
run_step $PRISMA migrate deploy --schema "$SCHEMA"
DEPLOY_STATUS=$?
set -e

if [ "$DEPLOY_STATUS" -eq 124 ]; then
    fatal "FATAL: prisma migrate deploy timed out after ${STEP_TIMEOUT}s." \
        "The database accepted the connection but did not finish the migration." \
        "Check for a lock held by another migrating service, or a database under load."
    exit 1
...
if [ "$DRIFT_STATUS" -eq 2 ]; then
    if drift_override_set; then
        ...
        fatal "FATAL: the database does not match schema.prisma (see the diff above)." \
    ...
    if [ "$DRIFT_STATUS" -eq 124 ]; then
        fatal "FATAL: the schema-drift check timed out after ${STEP_TIMEOUT}s." \
            "The database accepted the connection but never answered. This is NOT confirmed drift."
    else
        fatal "FATAL: could not check for schema drift — prisma migrate diff exited $DRIFT_STATUS." \
    ...
    if drift_override_set; then
        echo "[worker] WARNING: OUTPOST_ALLOW_SCHEMA_DRIFT is set — starting unverified." >&2
    else
        echo "[worker] To boot without the guard, set OUTPOST_ALLOW_SCHEMA_DRIFT=1." >&2
        exit 1
    fi
fi
