 ...
 */
import { FunnelStage, FUNNEL_STAGE_ORDER, } from './types.js';
/**
 ...
 */
export function computeFunnelMetrics(members) {
    const stageCounts = {
    ...
    return {
        stageCounts,
        conversionRates: {
            joinedToContacted: conversionRate(stageCounts[FunnelStage.JOINED], stageCounts[FunnelStage.CONTACTED]),
            contactedToResponded: conversionRate(stageCounts[FunnelStage.CONTACTED], stageCounts[FunnelStage.RESPONDED]),
            respondedToMeetingBooked: conversionRate(stageCounts[FunnelStage.RESPONDED], stageCounts[FunnelStage.MEETING_BOOKED]),
        },
        totalMembers: total,
    };
}
