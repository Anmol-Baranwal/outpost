 ...
 */
export function matchCondition(condition: RuleCondition, ticket: RoutingTicket): number {
 ...
 */
function matchKeyword(condition: KeywordCondition, ticket: RoutingTicket): number {
    ...
    const matchedKeywords = condition.keywords.filter(
        (kw) => normalizedText.includes(kw.toLowerCase()),
    );
 ...
 */
function matchSource(condition: SourceCondition, ticket: RoutingTicket): number {
    const matches = condition.sources.some(
        (source) => source.toUpperCase() === ticket.source.toUpperCase(),
    );
 ...
 */
function matchAcv(condition: AcvCondition, ticket: RoutingTicket): number {
 ...
 */
function matchTicketType(condition: TicketTypeCondition, ticket: RoutingTicket): number {
    const matches = condition.ticketTypes.some(
        (tt) => tt.toUpperCase() === ticket.type.toUpperCase(),
    );
